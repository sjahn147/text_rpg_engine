"""
직접 DB 연결을 통한 시나리오 테스트
pytest-asyncio 대신 직접 asyncio.run() 사용
"""
import asyncio
import sys
import os
...
async def test_db_connection_scenario():
...
async def test_entity_data_scenario():
...
async def test_cell_data_scenario():
...
async def test_dialogue_data_scenario():
...
async def test_session_data_scenario():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            assert test_session_found, f"테스트 세션 {session_id}을 찾을 수 없습니다"
            logger.info(f"[SUCCESS] 테스트 세션 확인: {session_id}")
...
async def test_full_data_flow_scenario():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            assert len(session_data) > 0, "세션 데이터가 없습니다"
            logger.info(f"[SUCCESS] 세션 데이터 확인: {len(session_data)}개 세션")
...
async def run_all_scenarios():
    ...
    scenarios = [
        test_db_connection_scenario,
        test_entity_data_scenario,
        test_cell_data_scenario,
        test_dialogue_data_scenario,
        test_session_data_scenario,
        test_full_data_flow_scenario
    ]
    ...
    for i, scenario in enumerate(scenarios, 1):
        try:
            logger.info(f"--- 시나리오 {i}/{len(scenarios)} 실행 ---")
            await scenario()
            passed += 1
            logger.info(f"[SUCCESS] 시나리오 {i} 완료")
        except Exception as e:
            failed += 1
            logger.error(f"[FAILED] 시나리오 {i} 실패: {str(e)}")
    
    logger.info(f"=== 모든 시나리오 실행 완료 ===")
    logger.info(f"통과: {passed}/{len(scenarios)}")
    logger.info(f"실패: {failed}/{len(scenarios)}")
